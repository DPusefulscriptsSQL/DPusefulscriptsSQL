...
select isnull(compatibility_level,0) compatibility_level from master.sys.databases  where [name] ='soadb'

Select MajorVersion ,(select isnull(compatibility_level,0) compatibility_level from master.sys.databases  where [name] ='soadb') from (
SELECT 
  CASE 
     ...
     WHEN CONVERT(VARCHAR(128), SERVERPROPERTY ('productversion')) like '10.5%' THEN 2008
